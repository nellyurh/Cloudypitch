...
export const PrizePoolsList = () => {
  ...
  useEffect(() => { (async () => { try { const { data } = await api.get("/prize-pools"); setPools(data.pools || []); } catch (_) {} })(); }, []);
  return (
        ...
        {pools.map(p => (
          <Link key={p.id} to={`/prize-pool/${p.id}`} className="cp-surface p-4 hover:bg-white/5 flex items-center gap-3" data-testid={`pool-${p.id}`}>
            ...
            <div className="flex-1">
              ...
              <div className="text-xl font-extrabold text-cp-lime">₦{(p.amount_total_ngn || 0).toLocaleString()}</div>
            </div>
          </Link>
        ))}
      </div>
...
export const PrizePoolDetail = () => {
  ...
  useEffect(() => { (async () => { try { const { data } = await api.get(`/prize-pools/${id}`); setPool(data.pool); setWinners(data.winners || []); } catch (_) {} })(); }, [id]);
  if (!pool) return <div className="cp-surface p-6 text-sm" style={{ color: "var(--cp-text-muted)" }}>Loading…</div>;
  const payouts = pool.payout_structure || {};
  const total = pool.amount_total_ngn || 0;
  return (
    <div className="max-w-3xl mx-auto" data-testid="prize-pool-detail">
      ...
      <div className="cp-surface p-5">
        ...
        <div className="text-3xl font-extrabold text-cp-lime mt-2">₦{total.toLocaleString()}</div>
        <div className="text-xs mt-1" style={{ color: "var(--cp-text-muted)" }}>{new Date(pool.starts_at).toLocaleDateString()} → {new Date(pool.ends_at).toLocaleDateString()}</div>
      </div>

      <div className="cp-surface mt-3">
        <div className="cp-card-header normal-case"><span className="flex items-center gap-2 font-bold" style={{ color: "var(--cp-text)" }}><Trophy size={14} className="text-cp-lime"/> Payout Structure</span></div>
        <ul className="divide-y" style={{ borderColor: "var(--cp-border)" }}>
          {Object.entries(payouts).map(([rank, share]) => (
            <li key={rank} className="px-3 py-2 flex items-center justify-between text-sm">
              <span>Rank {rank}</span>
              <span className="font-bold tabular-nums">₦{Math.round(total * share).toLocaleString()} <span className="text-[10px]" style={{ color: "var(--cp-text-muted)" }}>({Math.round(share * 100)}%)</span></span>
            </li>
          ))}
        </ul>
      </div>
          ...
          <ul className="divide-y" style={{ borderColor: "var(--cp-border)" }}>
            {winners.map(w => (
              <li key={w.id} className="px-3 py-2 flex items-center gap-2 text-sm">
                ...
                <span className="text-cp-lime font-bold">₦{w.amount_won_ngn.toLocaleString()}</span>
              </li>
            ))}
          </ul>
        </div>
