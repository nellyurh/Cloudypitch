...
async def my_wallet(user: dict = Depends(a.get_current_user)):
    """Returns the NGN wallet (`user_wallets.balance_ngn` + totals), the
    legacy USD spend wallet (`users.wallet_balance_usd_cents`, frozen — only
    new top-ups credit coins), AND the **coin** balance (`users.coins`).
    Cards are bought with coins only (1 NGN = 1 coin; $1 = 1,370 coins with
    a 5% bonus for crypto)."""
    wallet = await _ensure_wallet(user["id"])
    udoc = await get_db().users.find_one(
        {"id": user["id"]},
        {"_id": 0, "wallet_balance_usd_cents": 1, "coins": 1},
    ) or {}
    return {
        "wallet": wallet,
        "wallet_balance_usd_cents": int(udoc.get("wallet_balance_usd_cents") or 0),
        "coins": int(udoc.get("coins") or 0),
    }
